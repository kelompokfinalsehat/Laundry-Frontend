...
export function InviteEmployeeContent() {
  ...
  const handleSubmit = async (
    values: InviteEmployeeFormValues,
  ) => {
    inviteEmployee.mutate(values, {
      onSuccess: () => {
        notifications.show({
          title: "Berhasil",
          message:
            "Undangan karyawan berhasil dikirim.",
          color: "green",
        });

        router.push(
          "/internal/super-admin/karyawan",
        );
      },
  ...
  return (
    <Stack gap="lg">
      <PageHeader
        ...
        description="Kirim undangan untuk membuat akun karyawan baru."
      />
