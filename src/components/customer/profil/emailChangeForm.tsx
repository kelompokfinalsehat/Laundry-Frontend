...
import { Stack, TextInput, Button, Alert, Text, Badge, Group } from "@mantine/core";
...
export function EmailChangeForm() {
  ...
  const { mutate, isPending, error } = useRequestEmailChange();
    ...
    <Stack gap="sm">
      ...
      {requestedEmail && (
        <Alert style={{ backgroundColor: "var(--color-primary-light)", color: "var(--color-primary)" }}>
          Link konfirmasi sudah dikirim ke <strong>{requestedEmail}</strong>. Email saat ini tetap
          aktif sampai kamu konfirmasi lewat link itu.
        </Alert>
      )}
      ...
      {user?.authProvider === "GOOGLE" ? (
        <Text size="sm" c="var(--color-text-secondary)">
          Email akun Google terkunci ke akun Google kamu, tidak bisa diganti dari sini.
        </Text>
      ) : (
